...
**Navigation:** Three native-stack screens (`src/navigation/RootNavigator.tsx`): `Library` → `LiveStage` (params: `songId`) → `AddSong` (params: `{ mode: 'create' } | { mode: 'edit'; songId }` — dual create/edit screen). The stack header is disabled everywhere — every screen builds its own header row, so don't reach for `navigation.setOptions`/header APIs when changing a screen's chrome.
...
**UI kit:** Nativewind (Tailwind for React Native) + React Native Reusables (RNR, shadcn/ui-style components) is the intended UI stack going forward, replacing the hand-rolled `src/ui/` kit (Button, Card, Input, Segmented, Tag, icons). `src/ui/` and most existing screens (`LiveStageScreen`, `AddSongScreen`, `LibraryScreen`, `SettingsSheet`, `QuickToolsFab`) still use the hand-rolled kit and are not yet migrated — treat them as legacy pending migration, not as the pattern to copy for new work. The Live Stage "Menu" drawer (`src/screens/live-stage/MenuDrawer.tsx` and its `MenuDrawer*Tab.tsx`/`SetlistBuildView.tsx` siblings) is the first RNR-based surface in the app and the reference example for the pattern: Tailwind `className` + `@/components/ui/*` for layout/text/controls, but raw `react-native-svg` icons (`src/ui/icons.tsx`) still get their `color` from `useTheme()` rather than a Tailwind class, since NativeWind doesn't remap SVG stroke/fill props in this project. Write new components/screens with Tailwind utility classes via the `className` prop and RNR components rather than adding to `src/ui/`.
